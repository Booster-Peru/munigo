...
import { getMandado, cancelMandado, getMandadoHistory, Mandado, MandadoStatus } from '../../services/mandadosService';
    ...
    return (
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        {history.length === 0 ? (
          ...
          history.map(m => {
            const meta = { PENDING: '#d97706', ACCEPTED: '#0d9488', IN_PROGRESS: '#2563eb', DELIVERED: '#059669', CANCELLED: '#dc2626' };
            return (
              <View key={m.id} style={styles.historyCard}>
                <View style={styles.historyRow}>
                  <Text style={styles.historyType}>{m.type}</Text>
                  <Text style={[styles.historyStatus, { color: meta[m.status] }]}>{STATUS_LABELS[m.status]}</Text>
                </View>
                <Text style={styles.historyDesc} numberOfLines={1}>{m.description}</Text>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.statusBanner}>
        <Ionicons
          name={(STATUS_ICONS[mandado?.status as MandadoStatus] || 'hourglass-outline') as any}
          size={28}
          color={theme.colors.primary}
        />
        <Text style={styles.statusBannerText}>{STATUS_LABELS[mandado?.status as MandadoStatus] || '…'}</Text>
      </View>
        ...
        {STATUS_STEPS.map((s, i) => {
          ...
          return (
            <View key={s} style={styles.stepRow}>
              ...
              <Text style={[styles.stepLabel, active && styles.stepLabelActive]}>{STATUS_LABELS[s]}</Text>
...
const styles = StyleSheet.create({
  ...
  stepLine: { position: 'absolute', left: 10, top: 30, width: 2, height: 16, backgroundColor: theme.colors.border },
  ...
  cancelBtn: {
    margin: 16, borderRadius: theme.roundness.medium,
