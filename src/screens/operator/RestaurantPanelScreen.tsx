...
const NEXT_ACTION: Partial<
  Record<
    OrderStatus,
    { action: 'accept' | 'preparing' | 'ready' | 'delivering' | 'delivered'; label: string }
  >
> = {
...
export default function RestaurantPanelScreen() {
  ...
  const handleAdvance = async (order: Order) => {
    ...
    Alert.alert(
      next.label,
      `¿Confirmar: "${next.label}" para pedido #${order.id.slice(0, 8).toUpperCase()}?`,
      [
