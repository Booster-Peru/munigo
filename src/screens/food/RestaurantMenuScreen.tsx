...
export default function RestaurantMenuScreen() {
  ...
  useEffect(() => {
    getRestaurant(restaurantId)
      .then(({ restaurant: r, menu: m }) => { setRestaurant(r); setMenu(m); })
  ...
  const handleOrder = async () => {
    if (!token) { Alert.alert('Error', 'Inicia sesión primero'); return; }
    if (!cartCount) { Alert.alert('Carrito vacío', 'Agrega al menos un item'); return; }
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.headerSub}>{restaurant?.category} · {restaurant?.open_time}–{restaurant?.close_time}</Text>
        </View>
        <View style={{ width: 36 }} />
      </View>
            ...
            <View style={styles.menuItem}>
              ...
              <View style={styles.menuItemActions}>
                {qty > 0 ? (
                  ...
                  <TouchableOpacity style={styles.addBtn} onPress={() => addToCart(item)} disabled={!item.is_available}>
                    <Ionicons name="add" size={20} color={item.is_available ? theme.colors.primary : theme.colors.textSecondary} />
                  </TouchableOpacity>
                )}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  qtyText: { fontSize: 16, fontWeight: '700', color: theme.colors.text, minWidth: 20, textAlign: 'center' },
  cartBar: {
